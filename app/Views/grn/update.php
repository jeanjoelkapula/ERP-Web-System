		...
		<div class="layout-content">
			...
			<div class="container-fluid flex-grow-1 container-p-y">
				<h4 class="font-weight-bold py-3 mb-4">
					<span class="text-muted font-weight-light">Goods Received Note /</span> Update
				</h4>
				<div class="card mb-4">
					...
					<div class="card-body">
						<?php
							...
							?>
						<form method="post" action="/grn/update/<?php if($ent_id != null){echo $ent_id;}?>" class="form-horizontal" autocomplete="off" id="grn-form" onsubmit="return validateForm()" >
							<input type="hidden" name="form_update_grn" value="true" />                                
							<div class="panel-body">
								<!-- Field -->
								<div class="form-group">
									<label class="col form-label">Delivery Date <span class="text-danger">*</span></label>
									<div class="col"><input type="text" class="form-control" id="datepicker" name="grn_date" value=<?php if($received_date != null){echo $received_date;}?> required></div>
								</div>
								<!-- Field -->
									...
									<div class = "col">
										...
										<?php                                     
											...
											foreach ($query->getResult() as $row): {
                                                ...
                                                if($hub_name == $row->HUB_NAME){echo " selected";}
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="col">
                                                        <div class="autocomplete" style="width:500px;">
                                                            <input id="purchase-search" type="text" class="form-control" placeholder="Purchase Order">
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class = "tabcontent" id = "requisition">
                                            <div class = "row" id = "requisition-search-group" >
                                                <div class="form-group">
                                                    ...
                                                    <div class="col">
                                                        <div class="autocomplete" style="width:500px;">
                                                            <input id="requisition-search" type="text" class="form-control" placeholder="Requisition">
                                                        </div>
                                            ...
                                            <div class="form-group" id = "requisition-info" style= "display: none">
                                                ...
                                                <label class="col" style = "color: #696969; line-height: 5px;">Requisition Item <label id ="label-requisition-item" style = "color: #ababab">&nbsp;&rarr;&nbsp;<?php if (isset($ebq_code))echo $ebq_code; ?></label></label>
                                            </div>
                                        </div>
                                ...
                                <!-- Table -->
								<div class = "form-group" id = "purchase-table" style="display:<?php if (($pid == null) || ($pid == 0)) echo 'none;'; ?>">
									...
									<div class = "col">
										<table id = "tbl_stock" class="table table-striped font-sm" >
                                            ...
                                            <input type = "hidden" name = "grn-type" id = "grn-type" value = "purchase">
                                            <input type = "hidden" name = "source-id" id = "source-id" value = "<?php echo $pid; ?>">
                                        </table>
									</div>
								</div>

                                <!-- Table -->
                                <div class = "form-group" id='requisition-table' style="display:<?php if (($rid == null) || ($rid == 0)) echo 'none;'; ?>">
                                    <label class="col form-label">Sub Items</label>
                                    <div class = "col">
                                        <table id = "tbl_req" class="table m-0 font-sm" style="">
                                            ...
                                            <input type = "hidden" name = "grn-type" id = "grn-type" value = "requisition">
                                            <input type = "hidden" name = "source-id" id = "source-id" value = "<?php echo $rid; ?>">
                                        </table>
                                    </div>
                                    </div>
                                </div>
								...
								<br/>
								<button type="button" class="btn btn-primary" id ="btn-grn-create">Update</button>                                                                    
							</div>
					</div>
					</form>
				</div>
...
<?php if($rid != null){echo "<script>var source = 'req';var sid = '$rid';</script>";}?>
<?php if($pid != null){echo "<script>var source = 'pur';var sid = '$pid';</script>";}?>
...
<script>
	...
	$(document).ready(function() {   
                ...
                success: function(requisition_info) {
                    ...
                    try {                        
                        ...
                        requisition_info.forEach(function(item, index) {
                            autoSuggestions[index] = item.REQUISITION_NO + ' : ' + item.REQUISITION_DATE                            
                        });
                ...
                $.ajax({
                    ...
                    success: function(purchase_stock_info) {
                        ...
                        purchase_stock_info.forEach(function(item, index) {
                            ...
                            <input type = "hidden" name = "prices[]" value = ${item.COST}>               
        ...
        $('#btn-requisition-add').click(function() {
            ...
            var splitInput = itemName.split(' : ');
            var requisition_id = splitInput[0];
            ...
            if (is_search_value_in_autocomplete > -1) {	            
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        'requisition-id': obj.REQUISITION_NO                 
                    },
        ...
        if(source == "req"){
            ...
            $.ajax({
                ...
                url: window.location.origin + '/requisition/autocomplete',
                data:{
                    'requisition-updatefill': 'true',
                    'sid': sid,                   
                },
                success: function(req_update) {
                    ...
                    document.getElementById("label-requisition-no").appendChild(document.createTextNode(req_update.REQUISITION_NO));
                    document.getElementById("label-requisition-date").appendChild(document.createTextNode(req_update.REQUISITION_DATE));  
                    document.getElementById("label-requisition-notes").appendChild(document.createTextNode(req_update.NOTES));  
                    
                    $.ajax({
                        type: "POST",
                        url: window.location.origin + '/requisition/autocomplete',
                        data: {
                            'requisition-stock-updatefill': 'true',
                            'requisition-id': req_update.REQUISITION_NO                 
                        },
                        success: function(requisition_stock_info) {

                        purchaseStock = requisition_stock_info;
                        //stores stockItems related to selected purchase order
                        requisition_stock_info.forEach(function(item, index) {                            
                            row = document.createElement('tr');                            
                            row.innerHTML = `                    
                            <td class="">${item.EBQ_CODE}</td>
                            <input type = "hidden" name = "ebqs[]" value = ${item.EBQ_CODE}>
                            ...
                            <td class="">-</td>
                            <td class="">-</td>    
                            <td class="" name = "switch-cell">
                                <input type = "checkbox" ${((item.APPROVED == 1) ? 'checked = checked' : '')} id = "check-${item.EBQ_CODE}" onClick = "ManageNoteText(this)" name = "check">
                                <input type = "hidden" id = "checkval-${item.EBQ_CODE}" ${((item.APPROVED == 1) ? 'value = "1"' : 'value = "0"')} name = "checkboxes[]">
                            </td>                            
                            <td class="" name = "note-cell">
                                <input type = "text" value = "${item.APPROVAL_NOTE}" class = "form-control" name = "textboxes[]" id = "text-${item.EBQ_CODE}">
                            </td>`;
                            ...
                            document.getElementById("grn-type").value = "requisition";
                            document.getElementById("source-id").value = req_update.REQUISITION_NO; 
                            id_set = true;
                        },
                        error: function(data) {
                        console.log(data);
        ...
        if(source == "pur"){
            ...
            $.ajax({
                ...
                success: function(req_update) {
                    ...
                    document.getElementById("label-purchase-id").appendChild(document.createTextNode(req_update.PURCHASE_ORDER_ID));
                    document.getElementById("label-purchase-date-o").appendChild(document.createTextNode(req_update.ORDER_DATE));  
                    document.getElementById("label-purchase-date-r").appendChild(document.createTextNode(req_update.DATE_REQUIRED));  
                    document.getElementById("label-purchase-via").appendChild(document.createTextNode(req_update.SHIP_VIA));  
                    document.getElementById("label-purchase-vname").appendChild(document.createTextNode(req_update.VENDOR_NAME));          
                    document.getElementById("label-purchase-vaddress").appendChild(document.createTextNode(req_update.VENDOR_ADDRESS));  
                    document.getElementById("label-purchase-pobox").appendChild(document.createTextNode(req_update.VENDOR_PO_BOX));  
                    document.getElementById("label-purchase-zip").appendChild(document.createTextNode(req_update.VENDOR_ZIP_CODE));  
                    document.getElementById("label-purchase-tcost").appendChild(document.createTextNode("R"+req_update.TOTAL));  
                    ...
                    $.ajax({
                        ...
                        url: window.location.origin + '/purchase/autocomplete',
                        data: {
                            'purchase-stock-updatefill': 'true',
                            'purchase-id': req_update.PURCHASE_ORDER_ID,                    
                        },
                        success: function(purchase_stock_info) {

                            purchaseStock = purchase_stock_info;
                            console.log(purchase_stock_info)
                            //stores stockItems related to selected purchase order
                            purchase_stock_info.forEach(function(item, index) {
                                ...
                                <td class="">${item.TOTAL}</td>                            
                                <td class="" name = "switch-cell">
                                    <input type = "checkbox" ${((item.APPROVED == 1) ? 'checked = checked disabled' : '')} id = "check-${item.EBQ_CODE}" onClick = "ManageNoteText(this)" name = "check">
                                    <input type = "hidden" id = "checkval-${item.EBQ_CODE}" ${((item.APPROVED == 1) ? 'value = "1"' : 'value = "0"')} name = "checkboxes[]">
                                </td>                            
                                <td class="" name = "note-cell">
                                    <input type = "text" value = "${item.APPROVAL_NOTE}" class = "form-control" name = "textboxes[]" id = "text-${item.EBQ_CODE}">
                            ...
                            document.getElementById("grn-type").value = "purchase";
                            document.getElementById("source-id").value = req_update.PURCHASE_ORDER_ID; 
                            id_set = true;
                        },
                        error: function(data) {
                        console.log(data);
