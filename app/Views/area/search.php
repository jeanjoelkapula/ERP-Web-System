    ...
    <div class="layout-content">
        ...
        <div class="container-fluid flex-grow-1 container-p-y">
            ...
            <h4 class="font-weight-bold py-3 mb-4">
                <span class="text-muted font-weight-light">Area /</span> Search
                <?php 
                    if(($ionAuth->inGroup('electrical_manager') || ($ionAuth->inGroup('admin')))){
                ?>
                    <span class="float-right">
                        <a href="/area/create" class="btn btn-primary"><i class="fas fa-plus"></i>&nbsp; New Area </a>
                    </span>
                <?php
                    }
                ?>
                      ...
                      <th>Area No</th>
                       ...
                       <?php foreach ($result->getResult('array') as $row): { ?> 
                                <tr>
                                    ...
                                    <td><?php echo $row['AREA_MANAGER'];?></td>
                                    <td><?php echo $row['EMAIL'];?></td>
                                    <td><?php echo $row['CONTACT_NUMBER'];?></td>
                                    <td>
                                        <form method="post" action="/area/update/<?php echo $row['AREA_NO']; ?>">
                                            <input type="hidden" name="form_update_status" value="true" /> 
                                            <input type="hidden" name="area_name" value="<?php echo  $row['AREA_NAME'];?>" />
                                            <input type="hidden" name="area_manager" value="<?php echo  $row['AREA_MANAGER'];?>" />
