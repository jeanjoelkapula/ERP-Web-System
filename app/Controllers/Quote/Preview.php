...
class Preview extends BaseController {
    ...
    {   
        ...
        $data["url"] = "/quote/preview/$entity_id";   
        if (!isset($entity_id) || ($entity_id <= 0)) {
            ...
            exit();
             ...
             if($stock_found){
                 $sql = "SELECT quote_stock.*, stock.DESCRIPTION FROM TBL_QUOTE_STOCK quote_stock JOIN TBL_STOCK stock ON stock.EBQ_CODE = quote_stock.EBQ_CODE WHERE QUOTE_ID = '$entity_id';";
                 $result = $this->db->query($sql);
                ...
                $stockSQL = "SELECT sum(quote_stock.QUANTITY*stock.AVG_COST*(1+(stock.MARKUP/100))) as total from TBL_QUOTE_STOCK quote_stock join TBL_STOCK stock on stock.EBQ_CODE = quote_stock.EBQ_CODE where quote_stock.QUOTE_ID = $entity_id GROUP BY QUOTE_ID";
                $stockResult = $this->db->query($stockSQL);
                $data['stock_total'] = $stockResult->getResult()[0]->total;
             }
         
        }               
        echo view('quote/quote_print_preview',$data);
